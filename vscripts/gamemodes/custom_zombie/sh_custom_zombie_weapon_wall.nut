    ...
    const string WEAPON_WALL_BUY_WEAPON      = "%use% to buy weapon"
    const string WEAPON_WALL_BUY_AMMO        = "%use% to buy ammo"
    ...
    {
    	if ( PlayerHasWeapon( GetLocalViewPlayer(), "mp_weapon_rspn101"/* weaponName */ ) )
    		return WEAPON_WALL_BUY_AMMO
    	return WEAPON_WALL_BUY_WEAPON
    }
#endif // CLIENT
    ...
    void function ServerWeaponWallUseSuccess( entity usableWeaponWall, entity player )
    {
        ...
        switch( GetSlotForWeapon(player, player.GetActiveWeapon( eActiveInventorySlot.mainHand ) ) )
        {
            case WEAPON_INVENTORY_SLOT_PRIMARY_0:
                if  ( secondary == null ) weapon = player.GiveWeapon( weaponName, WEAPON_INVENTORY_SLOT_PRIMARY_1 )
                else
                {
            ...
            case WEAPON_INVENTORY_SLOT_PRIMARY_1:
                if  ( primary == null ) weapon = player.GiveWeapon( weaponName, WEAPON_INVENTORY_SLOT_PRIMARY_0 )
                else
                {
            ...
            default:
                if ( primary == null && secondary == null ) weapon = player.GiveWeapon( weaponName, WEAPON_INVENTORY_SLOT_PRIMARY_0 )
            break
        }
    ...
    entity function CreateWeaponWall( asset mdl, vector pos, vector ang )
    {
        ...
        weaponWall.SetUsable()
        weaponWall.SetUsableByGroup( "pilot" )
        weaponWall.SetUsablePriority( USABLE_PRIORITY_HIGH )
